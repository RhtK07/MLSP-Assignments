...
def stft(time_signal, time_dim=None, size=400, shift=160,
...
#    frames = _samples_to_stft_frames(time_signal.shape[time_dim], size, shift)
    frames =  np.ceil((time_signal.shape[0] - size + shift) / shift).astype(np.int)
  #  print(frames)
...
#        window = np.pad(window, (0, size - window_length), mode='constant')
    ...
    time_signal_seg = segment_axis(time_signal, size,shift, axis=time_dim)
  #  print(time_signal_seg)
#    print(np.shape(time_signal_seg))

    letters = string.ascii_lowercase
    mapping = letters[:time_signal_seg.ndim] + ',' + letters[time_dim + 1] \
              + '->' + letters[:time_signal_seg.ndim]

    stft_matrix=np.empty((time_signal_seg.shape[0],256), dtype=np.complex)
    
    for i in range(time_signal_seg.shape[0]):
        temp=np.einsum('i,i->i',window,time_signal_seg[i,:])

        stft_matrix[i,:]=rfft(temp,511)
    


    return stft_matrix.T
#    return rfft(np.einsum(mapping, time_signal_seg, window),256)
...
def cutt(time_signal, time_dim=None, size=8000, shift=4000,
         window=signal.blackman, fading=True, window_length=None):
    """
...
#    print(time_signal)
 ...
 #   print(np.shape(time_signal_seg))
    ...
    return (np.einsum(mapping, time_signal_seg, window))

def join(stft_signal, size=8000, shift=4000,
          window=signal.blackman, fading=False, window_length=None):
    """
    Calculated the inverse short time Fourier transform to exactly reconstruct
    the time signal.
...
#    assert stft_signal.shape[1] == size // 2 + 1
    ...
    for j, i in enumerate(range(0, len(time_signal) - size + shift, shift)):
        time_signal[i:i + size] += window * np.real((stft_signal[j]))
