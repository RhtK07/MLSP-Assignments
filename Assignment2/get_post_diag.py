...
def get_train_data(data_class):
	...
	if data_class in train_file:        
		...
		data=np.log10(np.abs(stft(data1, fs, 0.025, 0.010))) #np.log10
                #feature=librosa.core.stft(data1,n_fft=512, hop_length=hop, win_length=window, window='hann', center=True, dtype=np.complex128, pad_mode='reflect')           
		Xtrain.extend(data)
...
def gmm(data,typea):
    ...
    for iter in range(n_iter):
        ...
        #print(np.sum(b,axis=0)[0:300])
        likelihood[iter]=np.sum(np.log(np.sum(b,axis=0,dtype=np.float128),dtype=np.float128))
        b = b/b.sum(axis=0)
        #print("b",b[0:2,0:50])
	...
	for j in range(N):
		...
		for i in range(data.shape[0]):
	        	...
	        	covariance[j]=covariance[j]+(1/np.sum(b[j]))*b[j,i]*np.diag(np.square(t).reshape((128,)))
                #print(covariance[j])
		#covariance[j]=(1/np.sum(b[j]))*covariance[j]
                #print(covariance[j])
    post_dir="/home/rohitk/Desktop/MLSP/a2/final_sub"+"/parametersd_"+str(N)
...
def test_error():
    ...
    #print(likelihood1)
    mean=np.load('/home/rohitk/Desktop/MLSP/a2/final_sub/parametersd_'+str(N)+'/mean_music.npy')
    alpha=np.load('/home/rohitk/Desktop/MLSP/a2/final_sub/parametersd_'+str(N)+'/alpha_music.npy')
    covariance=np.load('/home/rohitk/Desktop/MLSP/a2/final_sub/parametersd_'+str(N)+'/covariance_music.npy')
    mean1=np.load('/home/rohitk/Desktop/MLSP/a2/final_sub/parametersd_'+str(N)+'/mean_speech.npy')
    alpha1=np.load('/home/rohitk/Desktop/MLSP/a2/final_sub/parametersd_'+str(N)+'/alpha_speech.npy')
    covariance1=np.load('/home/rohitk/Desktop/MLSP/a2/final_sub/parametersd_'+str(N)+'/covariance_speech.npy')
    correct=0
    total=0
